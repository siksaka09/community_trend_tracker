...
col_x, col_reddit, col_fb = st.columns(3)
with col_x:
...
with col_fb:
    ...
    st.caption("ข้ามอัตโนมัติถ้าปิดไว้ในหน้า Settings หรือยังไม่ได้ตั้งค่า URL เพจ")
    if st.button("▶️ รัน scrape_facebook.py", use_container_width=True):
        run_script("scrape_facebook.py", "ดึงโพสต์ Facebook")

st.subheader("2) วิเคราะห์ AI (relevance / sentiment / หัวข้อ + ราคา/โปรโมชั่นของ Facebook)")
...
st.caption("ดึงจาก X → Reddit → Facebook (ข้ามแหล่งที่ปิดไว้) → วิเคราะห์ AI → สรุปเทรนด์")
