...
แล้วสรุปออกมาเป็นรายงาน: ปริมาณทวีตตามเวลา, สัดส่วน sentiment, หัวข้อฮิต, ทวีตที่กำลังไวรัล และ
**คาดการณ์แนวโน้ม** (ให้ Claude ช่วยตีความรูปแบบข้อมูลว่าถ้าเทรนด์นี้ดำเนินต่อไป มีสัญญาณอะไรที่ควรจับตา)

มี **Local Web Interface** (`app.py`) ให้ดูกราฟ/รันสคริปต์/แก้ตั้งค่าผ่านเบราว์เซอร์ได้เลย
ไม่ต้องเปิด terminal พิมพ์คำสั่งเอง — ดูวิธีใช้ที่หัวข้อ "เปิดใช้งาน Web Interface" ด้านล่าง
...
├── app.py            # Local Web Interface (Streamlit) — ดูกราฟ/รันสคริปต์/ตั้งค่าผ่านเบราว์เซอร์
...
## เกี่ยวกับส่วนคาดการณ์แนวโน้ม (ส่วนที่ 5)

ทำงานโดย**เทียบสถิติระหว่าง 2 ช่วงเวลา** (ครึ่งแรก vs ครึ่งหลังของข้อมูลที่มีทั้งหมด) — ปริมาณโพสต์
เปลี่ยนไปเท่าไหร่, sentiment เปลี่ยนไปทางไหน, หัวข้อไหนกำลังมาแรง/ซาลง — แล้วส่ง**สรุปตัวเลขเหล่านี้**
(ไม่ใช่โพสต์ดิบทั้งหมด) ให้ Claude ช่วยตีความเป็นรายงานภาษาไทย

**ข้อควรเข้าใจ:** นี่คือ**การอนุมานจากรูปแบบข้อมูล ไม่ใช่การพยากรณ์ที่แม่นยำ** AI มองรูปแบบตัวเลข
แล้วให้เหตุผลแบบมีเงื่อนไข ("ถ้าแนวโน้มนี้ดำเนินต่อ อาจ...") ไม่ใช่การทำนายอนาคตที่แน่นอน ควรใช้
ประกอบการตัดสินใจร่วมกับข้อมูลอื่น ไม่ควรเชื่อเป็นข้อเท็จจริง — ยิ่งเก็บข้อมูลได้นานและสม่ำเสมอ
มากเท่าไหร่ การเทียบแนวโน้มก็จะมีความหมายมากขึ้นเท่านั้น (ข้อมูลแค่ 1-2 วันจะสรุปอะไรได้จำกัดมาก)

ใช้โมเดล `claude-sonnet-5` สำหรับส่วนนี้ (ต่างจาก `ai_analyze.py` ที่ใช้ Haiku) เพราะเรียก API แค่
ครั้งเดียวต่อการรัน (ไม่ใช่ต่อโพสต์) ต้นทุนต่ำมากอยู่แล้ว จึงเลือกใช้โมเดลที่ให้เหตุผล/เขียนได้ดีกว่า
ปรับเป็น `claude-haiku-4-5-20251001` ได้ที่ตัวแปร `FORECAST_MODEL` ใน `analyze.py` ถ้าอยากประหยัดสุด

## เปิดใช้งาน Web Interface

แทนที่จะรัน `scrape_x.py`/`ai_analyze.py`/`analyze.py` แยกกันใน terminal สามารถเปิดหน้าเว็บ
สำหรับดูกราฟ, กดปุ่มรันสคริปต์, และแก้ตั้งค่าได้เลย:

```bash
streamlit run app.py
```

เบราว์เซอร์จะเปิดที่ `http://localhost:8501` อัตโนมัติ (ใช้ได้เฉพาะบนเครื่องตัวเอง ไม่ได้ expose
ออกอินเทอร์เน็ต) ในหน้าเว็บมี 4 แท็บ:

1. **📊 ภาพรวมเทรนด์** — กราฟปริมาณโพสต์ตามเวลา, สัดส่วน sentiment, หัวข้อฮิต, ตารางทวีตไวรัล
   (พร้อมลิงก์กดไปดูทวีตจริงได้)
2. **🔮 คาดการณ์แนวโน้ม** — โชว์รายงานล่าสุด พร้อมปุ่มสร้างใหม่ (เรียก Claude 1 ครั้ง)
3. **🔄 ดึงข้อมูล/วิเคราะห์** — ปุ่มรัน `scrape_x.py`, `ai_analyze.py`, หรือ `run_all.py` แบบไม่ต้อง
   เปิด terminal เอง มี log แสดงผลหลังรันเสร็จ
4. **⚙️ ตั้งค่า** — แก้ `TWITTER_HANDLES`, คีย์เวิร์ด, โหมดกรอง, จำนวนทวีตต่อรอบ ได้จากหน้าเว็บ
   แล้วบันทึกกลับลงไฟล์ `.env` อัตโนมัติ (API keys ไม่แสดง/แก้ในหน้านี้ เพื่อความปลอดภัย —
   ต้องแก้ในไฟล์ `.env` โดยตรง)

**หมายเหตุ:** การรันสคริปต์จากปุ่มในเว็บใช้เวลาเท่ากับรันผ่าน terminal (ต้องรอ Apify/Claude
ตอบกลับจริง) ระหว่างรันห้ามปิดหน้าเว็บ และหลังรันเสร็จต้องกลับไปที่แท็บภาพรวมแล้วรีเฟรชหน้า
(กด R) เพื่อดูข้อมูลล่าสุด เพราะ Streamlit ไม่ได้อัปเดตอัตโนมัติแบบเรียลไทม์
